...
//			problem12.Solution();
    ...
    public class Problem11
    {
        ...
        private readonly string[] gridStrings;
        ...
        public Problem11()
        {
            ...
            seperators[1] = '\n';
            seperators[2] = '\r';
            gridStrings = gridString.Split(seperators, StringSplitOptions.RemoveEmptyEntries);
            gridDimension = (int) Math.Sqrt(gridStrings.Length);
            intGrid = new int[gridDimension][];
            for (int i = 0; i < gridDimension; i++)
            {
                intGrid[i] = new int[gridDimension];
            }

            int s = 0;
            for (int i = 0; i < gridDimension; i++)
            {
                for (int j = 0; j < gridDimension; j++, s++)
                {
                    intGrid[i][j] = int.Parse(gridStrings[s]);
                }
            }
        }
        ...
        {
            FindGreatestVerticalProduct();
            FindGreatestHorizontalProduct();
            FindGreatestDiagonalDownProduct();
            FindGreatestDiagonalUpProduct();
        ...
        public void FindGreatestVerticalProduct()
        {
            for (int i = 0; i < gridDimension; i++)
            {
                ...
                {
                    if (i+3 < gridDimension)
                    {
                        int product = 0;
                        product = intGrid[i][j] * intGrid[i + 1][j] * intGrid[i + 2][j] * intGrid[i + 3][j];
        ...
        public void FindGreatestHorizontalProduct()
        {
            for (int i = 0; i < gridDimension; i++)
            {
                for (int j = 0; j < gridDimension; j++)
                {
                    if (j + 3 < gridDimension)
                    {
                        int product = 0;
                        product = intGrid[i][j] * intGrid[i][j + 1] * intGrid[i][j + 2] * intGrid[i][j + 3]; 
                        if (product > greatestProduct)
                        {
                            greatestProduct = product;
                        }
        ...
        public void FindGreatestDiagonalDownProduct()
        {
            for (int i = 0; i < gridDimension; i++)
            {
                for (int j = 0; j < gridDimension; j++)
                {
                    if ((j + 3 < gridDimension) && (i + 3 < gridDimension))
                    {
                        int product = 0;
                        product = intGrid[i][j] *
                                  intGrid[i + 1][j + 1] *
        ...
        public void FindGreatestDiagonalUpProduct()
        {
            for (int i = gridDimension - 1; i >= 0; i--)
            {
                for (int j = 0; j < gridDimension; j++)
                {
                    if ((j + 3 < gridDimension) && (i - 3 >= 0))
                    {
                        int product = 0;
                        product = intGrid[i][j] *
                                  intGrid[i - 1][j + 1] *
